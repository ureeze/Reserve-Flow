...
from app.service import ReservationExtractor
...
class FakeStructuredModel:
    ...
    async def ainvoke(self, messages):
        return ReservationExtractionResponse(
...
@pytest.mark.asyncio
async def test_extractor_returns_structured_reservation_condition():
    """구조화된 LLM 결과를 Spring Boot 계약과 같은 필드명으로 반환하는지 검증한다."""
    extractor = ReservationExtractor(structured_model=FakeStructuredModel())

    result = await extractor.extract(
        ReservationExtractionRequest(
