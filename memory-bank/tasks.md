...
- [T-011] Hold 생성 요청 API 구현 (Jira: RF-10, 브랜치: `feature/RF-10-hold-request`, PR: https://github.com/ureeze/Reserve-Flow/pull/31, 검증: `.\backend\gradlew.bat test` 전체 통과(신규 4건), V6 `hold_requests` 마이그레이션·HoldRequest 엔티티/상태 enum/Repository·요청/응답 DTO·HoldRequestService(회원·slot 확인 후 PENDING 저장 + `HOLD_REQUESTED` Outbox 이벤트 동일 트랜잭션)·`POST /api/v1/hold-requests`(202 Accepted)·`SLOT_001` ErrorCode·`OutboxEventRecorder.record(...)` 리네임 포함)- [T-009] 예약 제공자 검색 API 구현 (Jira: RF-8, 브랜치: `feature/RF-8-booking-provider-search`, PR: https://github.com/ureeze/Reserve-Flow/pull/27, 검증: `.\backend\gradlew.bat test` 통과(신규 7건), QueryDSL 동적 검색·`GET /api/v1/booking-providers`·size clamp·partySize `VALIDATION_002`, Jira 상태 변경: `검토 중` 전환 완료, Slack: GitHub/Jira 공식 앱)
