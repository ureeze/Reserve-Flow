# Decisions

ADR 원본은 Notion ADR 문서를 기준으로 한다.

- Notion ADR: https://app.notion.com/p/3828a222ce3c80b481ebfb7fcfb3476c

이 파일은 Codex 작업 중 빠르게 참조하기 위한 로컬 인덱스이자 요약 캐시다. Notion ADR과 이 파일이 충돌하면 Notion ADR을 우선한다.

## ADR Index

- ADR-001 Bearer Token 기반 JWT 인증
- ADR-002 Kafka + Transactional Outbox 기반 비동기 처리
- ADR-003 PostgreSQL + Redis 역할 분리
- ADR-004 범용 예약 제공자 모델 사용
- ADR-005 주요 엔티티 PK로 UUID 사용
- ADR-006 Hold와 Promotion Hold 만료 시간 5분
- ADR-007 Spring Boot 백엔드 우선 개발
- ADR-008 Jira 기반 개발 진행 및 GitHub PR 운영 흐름
- ADR-009 Python FastAPI + LangChain 기반 자연어 예약 해석 서비스
