...
- 작업 종료 전 `current-state.md`와 `tasks.md`를 갱신한다.
...
기본값은 `로컬 구현 + 테스트 + Memory Bank 갱신`이다. GitHub push, PR 생성, Jira 상태 변경, Slack 메시지 전송은 실행계획에 포함하고 사용자 확인을 받은 경우에만 수행한다.
...
- 작업 시작 시 Jira 티켓을 확인한다. Jira 상태를 `진행 중`으로 변경하는 작업은 실행계획에 포함하고 사용자 확인을 받은 경우에만 수행한다.
...
- Slack 공유는 사용자가 명시적으로 요청했거나 주요 마일스톤 완료, PR 생성, 배포 준비처럼 공유 가치가 있는 경우에만 수행한다.
- Slack 메시지는 작업 요약, Jira 링크, PR 링크, 검증 결과, 다음 액션을 짧게 포함한다.
